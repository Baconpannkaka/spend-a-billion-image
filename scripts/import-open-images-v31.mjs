...
function parseArgs(argv) {
  const result = { scope: "sample", limit: 30, approvalMode: "review", overwrite: false, dryRun: false, candidates: 12, delayMs: 300 };
...
function selectProducts(catalogs, options, existingMap, reviewMap, feedback) {
  ...
  const ordered = Object.fromEntries(Object.entries(buckets).map(([key, values]) => [key, selectStratified(values, values.length)]));
  const chosen = [];
  const ids = new Set();
  function take(reason, amount) {
    let taken = 0;
    while (ordered[reason]?.length && chosen.length < requested && taken < amount) {
      const product = ordered[reason].shift();
      if (!ids.has(product.id)) { chosen.push(product); ids.add(product.id); taken += 1; }
    }
  }
  take("rejected", Math.ceil(requested * 0.2));
  take("upgrade-retry", Math.ceil(requested * 0.7));
  take("error-retry", Math.ceil(requested * 0.05));
  take("fresh", requested - chosen.length);
  while (chosen.length < requested) {
    const before = chosen.length;
    for (const reason of ["upgrade-retry", "rejected", "error-retry", "fresh"]) take(reason, requested - chosen.length);
  ...
  for (const entry of Object.values(feedback.products ?? {})) {
    for (const decision of ["approved", "rejected"]) {
      for (const item of entry?.[decision] ?? []) {
        const text = normalizeForImageSearch(`${item.commonsTitle ?? item.sourceTitle ?? ""} ${item.sourceUrl ?? ""}`);
...
function removeRejectedCandidates(productId, ranked, feedback) {
  ...
  const blockedTitles = new Set(rejected.map((entry) => entry.commonsTitle ?? entry.sourceTitle).filter(Boolean));
...
function rankMerged(target) {
  return [...target.values()].sort((a, b) => b.score - a.score || ((b.width ?? 0) * (b.height ?? 0)) - ((a.width ?? 0) * (a.height ?? 0)));
}

function openverseQueries(queries) {
  return unique([queries[1], queries[3], queries[5], queries[0], queries[2], queries[6]]).slice(0, 5);
}

function addDiagnostics(report, sourceType, diagnostics) {
  const target = report.sourceDiagnostics[sourceType];
  target.raw += diagnostics.raw;
  target.licenseEligible += diagnostics.licenseEligible;
  target.accepted += diagnostics.accepted;
  target.filteredBeforeRanking += diagnostics.filteredBeforeRanking;
  target.filteredAsIrrelevant += diagnostics.filteredAsIrrelevant;
}
...
async function searchSource({ product, queries, sourceType, search, merged, feedback, learnedBadTerms, report, options }) {
  for (const query of queries) {
    ...
    const decorated = candidates.map((candidate) => ({ ...candidate, matchedQuery: query }));
    const { ranked, diagnostics } = rankCandidatesWithDiagnostics(product, decorated, { learnedBadTerms });
    addDiagnostics(report, sourceType, diagnostics);
    const afterFeedback = removeRejectedCandidates(product.id, ranked, feedback);
    report.feedbackFiltered += ranked.length - afterFeedback.length;
    mergeCandidates(merged, afterFeedback);
    const best = rankMerged(merged)[0];
    if (sourceType === "commons" && best?.sourceType === "commons" && best.score >= 138) break;
    if (sourceType === "openverse" && best?.sourceType === "openverse" && best.score >= 115) break;
    if (options.delayMs) await sleep(options.delayMs);
  }
}

async function saveCandidateImage(product, ranked, outputFile, report) {
  let lastError;
  for (const candidate of ranked.slice(0, 5)) {
    ...
    try {
      ...
      await rm(tempFile, { force: true });
      report.downloads.succeeded += 1;
      return { candidate, dimensions };
    } catch (error) {
      lastError = error;
      report.downloads.failed += 1;
      const message = error instanceof Error ? error.message : String(error);
      if (message.includes("HTTP 406")) report.downloads.http406 += 1;
      console.warn(`  Kandidat från ${candidate.sourceType ?? "okänd källa"} kunde inte sparas: ${message}`);
    }
  }
...
const [luxury, everyday, manifest, review, overrides, feedback] = await Promise.all([
  ...
  readJson(MANIFEST_FILE, { version: 3, generatedAt: "", images: [] }),
  readJson(REVIEW_FILE, { version: 4, generatedAt: "", items: [] }),
...
const report = {
  ...
  sourceDiagnostics: {
    commons: { raw: 0, licenseEligible: 0, accepted: 0, filteredBeforeRanking: 0, filteredAsIrrelevant: 0 },
    openverse: { raw: 0, licenseEligible: 0, accepted: 0, filteredBeforeRanking: 0, filteredAsIrrelevant: 0 },
  },
  downloads: { attempted: 0, succeeded: 0, failed: 0, http406: 0 },
  results: [],
};
...
for (let index = 0; index < selected.length; index += 1) {
  ...
  try {
    ...
    const previousNoMatchVersion = previousReview?.status === "no-match" ? Number(previousReview.searchVersion ?? 0) : 0;
    const skipRepeatedCommons = previousNoMatchVersion >= 3 && previousNoMatchVersion < IMAGE_SEARCH_VERSION;

    if (override.commonsTitle) {
      report.sourceRequests.commons += 1;
      const exact = await getCommonsFile(override.commonsTitle);
      const decorated = exact ? [{ ...exact, matchedQuery: `override:${override.commonsTitle}` }] : [];
      const { ranked, diagnostics } = rankCandidatesWithDiagnostics(product, decorated, { learnedBadTerms });
      addDiagnostics(report, "commons", diagnostics);
      mergeCandidates(merged, removeRejectedCandidates(product.id, ranked, feedback));
    } else {
      if (!skipRepeatedCommons) {
      ...
      const bestAfterCommons = rankMerged(merged)[0];
      if (!bestAfterCommons || bestAfterCommons.score < 118) {
    ...
    if (!ranked[0]) {
      reviewMap.set(product.id, {
        ...
        notes: ["Bildsök v3.1 hittade ingen tillräckligt relevant kandidat. Diagnostiken i import-rapporten visar hur många Openverse-resultat som filtrerades bort."],
      });
      report.noMatch += 1;
    ...
    if (!options.dryRun) {
      const saved = await saveCandidateImage(product, ranked, outputFile, report);
    ...
    const imageRecord = {
      ...
      importedAt: now, commonsTitle: selectedCandidate.sourceType === "commons" ? selectedCandidate.title : undefined, sourceTitle: selectedCandidate.title,
    ...
    reviewMap.set(product.id, {
      ...
      notes: imageStatus === "approved"
        ? ["Bilden godkändes genom ett uttryckligt manuellt importläge."]
        : [`Bildsök v3.1 valde bästa kandidaten från ${selectedCandidate.sourceType === "openverse" ? "Openverse" : "Wikimedia Commons"}. Automatisk godkänning är avstängd.`],
    });
    report.imported += 1;
...
console.log(`Klart v${IMAGE_SEARCH_VERSION}: ${report.imported} importerade, ${report.noMatch} utan träff, ${report.errors} fel. Commons ${report.sourceRequests.commons} sökningar/${report.sourceMatches.commons} valda, Openverse ${report.sourceRequests.openverse} sökningar/${report.sourceMatches.openverse} valda.`);
console.log(`Diagnostik: ${JSON.stringify(report.sourceDiagnostics)}. Nedladdning: ${JSON.stringify(report.downloads)}.`);
